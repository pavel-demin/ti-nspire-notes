...
function connectionCallback(p, e)
    if e == asi.CONNECTED then
        port = p
        state = 3
        asi.stopScanning()
        p:write('CONNECT RGB\n')
    elseif e == asi.DISCONNECTED then
        port = nil
        state = 1
...
function portFoundCallback(p)
    state = 2
    p:connect(connectionCallback)
end

function asiStateCallback(s)
    if s == asi.ON then
        state = 1
        asi.startScanning(portFoundCallback)
    end
...
function on.timer()
    if active and port then
        update()
    end
end

function on.paint(gc)
    local n = port and port:getName() or 'none'
    gc:drawString('port: ' .. n, 120, 100)
end
